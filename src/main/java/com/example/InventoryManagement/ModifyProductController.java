...
/**
 * The controller for the modify product screen
 * @author Ohzecodes
 */
public class ModifyProductController implements Initializable {
     ...
     *  The bottom list populates the product's Associated parts when saving
     */
    private  void initApartTable(Product p){
    ...
    private boolean validate(){
        ...
        if (!a) {
            ...
            System.out.println("Price is Empty or not a Double ");
    ...
    /**
     * Populate the text fields with the information from the product that was passed. Copies the associated parts Of the product in to bo the bottomList.
     * @param p Product that you want to modify
     * @throws NullPointerException this happens if the part is null or not selected.
     */
    public void set(Product p) throws  NullPointerException{
        if(p==null){
         throw new NullPointerException("product cant be null"); // not that it ever would be null

        }else{
            productid.setText(String.valueOf(p.getId()));
            productname.setText(p.getNameProduct());
            productinv.setText(String.valueOf(p.getStockProduct()));
            productprice.setText(String.valueOf(p.getPriceProduct()));
            productmax.setText(String.valueOf(p.getMaxProduct()));
            productmin.setText(String.valueOf(p.getMinProduct()));
            initparttable();
     ...
     * @param actionEvent An Event representing some type of action.
      */
    public void addToAssociated(ActionEvent actionEvent) {
    ...
    public void rmAssociated(ActionEvent actionEvent) {
        Part p= (Part) associatedparts.getSelectionModel().getSelectedItem();
        if(p==null){
            errors.setStyle("-fx-text-fill: red");
            System.out.println("ERROR:select a part");
            errors.setText( "Errors:\n\t"+"Select a part to add to remove");
        }else{
            errors.setText("");
